...
$sqlNotes = "
    SELECT subject, grade_value, grade_scale, created_at
    FROM grades
...
<head>
  <meta charset="UTF-8">
  <title>ScoliOse – Suivi des notes</title>
  ...
  <section class="profile-layout">
    <section class="profile-column">
      <article class="profile-card">
        <h2>Suivi des notes<?= $className ? ' – ' . htmlspecialchars($className) : '' ?></h2>

        <?php if (empty($notes)): ?>
          <p>Aucune note enregistrée pour le moment.</p>
        <?php else: ?>
          <ul class="profile-grades-list">
            <?php foreach ($notes as $n): ?>
              <li class="profile-grade-item">
                <span class="profile-grade-subject">
                  <?= htmlspecialchars($n['subject']) ?>
                </span>
                <span class="profile-grade-value">
                  <?= htmlspecialchars($n['grade_value']) ?>/<?= htmlspecialchars($n['grade_scale']) ?>
                </span>
                <span class="profile-grade-date">
